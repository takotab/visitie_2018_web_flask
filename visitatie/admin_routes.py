...
def admin_decide():
    ...
    return render_template(
        "admin_decide.html", title="Admin", form=form, return_str=return_str
    )
...
def deel_indeling(regio=1):
    ...
    for user in users:
        did_go_to = [
            _user.vorig_name_code
            for _user in users
            if _user.vorig_bezoekende_praktijk == user.vorig_name_code
        ]
        if len(did_go_to):
            list_of_users[user.vorig_name_code] = {
                "constraint": [str(user.vorig_bezoekende_praktijk), str(did_go_to[0])],
            }
        else:
            list_of_users[user.vorig_name_code] = {
...
def regio(regio):
    ...
    return render_template(
        "regio.html",
        title="regio " + str(regio),
        user=[("id", "to", True), ("id", "to", False), ("id", "to", True)],
    )
