...
class FormNumbers(FlaskForm):
    ...
    ander_meet = StringField(
        "Welk ander meetinstrument gebruikt u?", validators=[DataRequired()]
    )
    anders_start = FloatField("Anders start", validators=[DataRequired()])
    anders_end = FloatField("Anders start", validators=[DataRequired()])

    aantal_behandelingen = IntegerField(
        "Aantal behandelingen?", validators=[DataRequired()]
    )
    Nog_een = SelectField(
        "Wilt u nog een patiënt invoeren?",
        choices=[("1", "Ja"), ("0", "Nee")],
        validators=[DataRequired()],
    )
